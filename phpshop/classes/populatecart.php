<?php
	...
	if (isset($_SESSION["cartid"])) {
		...
		while($i<$number){
			...
			echo '<input type="number" class="form-control text-center my-4" value = "'.htmlspecialchars($_SESSION["cartqtys"][$j]).'" onChange="updateCartQty(this.value,'.htmlspecialchars($_SESSION["cartitems"][$j]).')"></td>
			
			<td data-th="Subtotal" class="text-center my-7"><p>₡'.htmlspecialchars(number_format($subtotal, 2, '.', ',')).'</p></td>
			<td class="actions" data-th="">
				
				<button class="btn btn-danger btn-sm" role="button" onClick="deleteCartItem('.htmlspecialchars($_SESSION["cartitems"][$j]).')"><i class="fas fa-trash-alt" title="Eliminar"></i></button>		
		...
		<br><tfoot><br>
								...
								<tr>
									...
									<td class="hidden-xs text-center"><strong>₡'.htmlspecialchars(number_format($_SESSION["grantotal1"], 2, '.', ',')).' </strong></td>
									<td colspan="1" class="hidden-xs"></td>
								...
								<tr>
									...
									<td class="hidden-xs text-center"><strong>₡'.htmlspecialchars(number_format($iva, 2, '.', ',')).' </strong></td>
									<td colspan="1" class="hidden-xs"></td>
								...
								<tr>
									...
									<td class="hidden-xs text-center"><strong>₡'.htmlspecialchars(number_format($final, 2, '.', ',')).' </strong></td>
									<td colspan="1" class="hidden-xs"></td>
